...
class ServiceController extends AppController {
	...
	public function getServiceByName(){
		...
		$query = "SELECT * FROM subclasses WHERE name LIKE '{$name}%';";
...
public function getProviderByName(){
		
		$this->layout = '';
		
		$name = $this->request->data['searchService'];
		
		$query = "SELECT * FROM providers WHERE fancy_name LIKE '{$name}%';";
			...
			$servicesArr = unserialize(utf8_decode($this->AccentialApi->urlRequestToGetData('General', 'query', $params)));
			if($servicesArr != ''){
                            $this->set("providers", $servicesArr);
                        }
...
//            $organize[$service['classes']['name']]['name'] = $service['classes']['name'];
    ...
    private function addService($serviceinfo, $company) {
        ...
        if (empty($return)) {
            ...
            $serviceID = $this->AccentialApi->urlRequestToGetData('Services', 'query', $params)[0]['services']['id'];
